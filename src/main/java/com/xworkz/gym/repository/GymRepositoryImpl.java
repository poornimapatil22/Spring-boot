...
public class GymRepositoryImpl implements GymRepository {
    ...
    public AdminEntity getAdminDeatilsByEmail(String email) {
        ...
        try {
            List<AdminEntity> list = em.createNamedQuery("getAllByEmail", AdminEntity.class)
            ...
            log.info("Fetched AdminEntity for email {}: {}", email, adminEntity);
        } catch (Exception e) {
            log.error("Error fetching AdminEntity by email: {}", email, e);
        } finally {
            em.close();
        }
